...
def _gen_identity_entries(
    ...
    """
    Generate sparse entries for an identity matrix A (m x k),
    assuming m == k and both are multiples of block size.
    """
    if m != k:
        raise ValueError("Identity test requires m == k")
    if m % block_m != 0 or k % block_k != 0:
        raise ValueError("Identity generator assumes aligned M and K")

    entries: List[Tuple[int, int, float]] = []

    for i in range(m):
        # 1-based indices for MatrixMarket
        entries.append((i + 1, i + 1, 1.0))
...
def gen_case(out_root: str, spec: CaseSpec, seed: int) -> None:
    ...
    B_matrix_path = "{}/x2_output_matrix.txt".format(sample_dir)
    golden_matrix_path = "{}/golden_matrix.txt".format(sample_dir)
    print_2d_matrix(b.flatten(), B_matrix_path, spec.k, spec.n)
    print_2d_matrix(golden.flatten(), golden_matrix_path, spec.m, spec.n)
# 打印二维的矩阵
...
def main() -> None:
    ...
    ap.add_argument("--out", default="/root/autodl-tmp/12_24_bcsr/cann-bcsr/temp_input", help="output root directory")
    ap.add_argument("--seed", type=int, default=20251213, help="random seed")
    args = ap.parse_args()

    specs = [
        CaseSpec(name="aligned_16_with1_B", m=16, k=16, n=16),
