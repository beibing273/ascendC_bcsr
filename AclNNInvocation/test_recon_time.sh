...
function main {
    ...
    for mtx_file in $(find $INPUTS_DIR -name "*.mtx"); do
        ...
        msprof op simulator --output=./msprof_out  ./output/execute_spmm_op $m $k $n $window_num $block_num $input_row_ptr $input_col $input_values $input_b $output_c $category $sample_name $MODE $input_ref $con_thres
        if [ $? -ne 0 ]; then
